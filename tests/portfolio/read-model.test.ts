...
describe("priced portfolio read models", () => {
  ...
  it("collapses linked transfers and trades into logical operations", async () => {
    ...
    await createTransferMutation({ assetId: btc.id, fromAccountId: main.id, toAccountId: empty.id, quantity: "0.1", feeQuantity: "0.001", currency: "EUR", executedAt: new Date("2026-08-06") }, new PortfolioRepository(testDb.prisma));
    ...
    try {
      ...
      expect(grouped.find((row) => row.operationKind === "TRANSFER")).toEqual(expect.objectContaining({
        quantity: "0.101",
        feeQuantity: "0.001",
        inputFeeQuantity: "0.001",
        feeQuantityLabel: "0.001 BTC",
      }));
    } finally {
