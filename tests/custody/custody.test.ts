...
describe("custodian persistence", () => {
  ...
  it("creates and edits custodians and aggregates multiple linked accounts", async () => {
    const repository = new CustodyRepository(db.prisma);
    const saved = await saveCustodianMutation({ name: "Bybit", category: CustodianCategory.EXCHANGE }, repository);
    ...
    await assignAccountCustodianMutation({ accountId: spot.id, custodianId: saved.custodian.id }, repository);
    await assignAccountCustodianMutation({ accountId: earn.id, custodianId: saved.custodian.id }, repository);
    await saveCustodianMutation({ id: saved.custodian.id, name: "Bybit Global", category: CustodianCategory.EXCHANGE, description: "Counterparty" }, repository);
