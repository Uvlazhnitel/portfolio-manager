...
export async function getPerformanceReadModel({
  ...
  const advanced = calculateAdvancedPerformance({
    ...
    history: history.map((point) => ({
      date: point.date,
      portfolioValue: point.exactPortfolioValue ?? point.portfolioValue,
      investmentGain: point.exactInvestmentGain ?? point.investmentGain,
      externalContributions: point.exactExternalContributions ?? point.externalContributions,
      externalWithdrawals: point.exactExternalWithdrawals ?? point.externalWithdrawals,
    ...
    current: {
      date: currentDate,
      portfolioValue: portfolio.missingPriceSymbols.length === 0
        ? portfolio.exactTotalValue ?? portfolio.totalValue
        : null,
      investmentGain: analytics.exactInvestmentGain,
      externalContributions: analytics.exactExternalContributions,
      externalWithdrawals: analytics.exactExternalWithdrawals,
