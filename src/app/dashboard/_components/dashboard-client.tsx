...
function RecentActivity({ dashboard }: { dashboard: DashboardReadModel }) {
  return <Card><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">Recent activity</h2><p className="mt-1 text-sm text-muted">Latest five portfolio transactions.</p></div><Link href="/portfolio" className="text-sm text-primary hover:underline">View all</Link></div>{dashboard.recentActivity.length ? <div className="mt-5 divide-y divide-border">{dashboard.recentActivity.map((item) => <div key={item.id} className="flex flex-col gap-2 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex items-center gap-2"><Badge tone={item.type === "SELL" ? "destructive" : "neutral"}>{formatType(item.type)}</Badge><p className="truncate font-medium">{item.assetName} · {item.symbol}</p></div><p className="mt-1 text-xs text-muted">{item.accountName} · {formatUtcDate(item.executedAt)}</p></div><div className="text-left sm:text-right"><p className="font-medium">{formatQuantity(item.quantity)} {item.symbol}</p><p className="mt-1 text-xs text-muted">{item.pricePerUnit ? `${formatCurrency(item.pricePerUnit, item.currency)} / unit` : "No acquisition price"}</p></div></div>)}</div> : <InlineEmpty title="No activity yet" description="Transactions will appear here after you add an initial balance or trade." />}</Card>;
}
