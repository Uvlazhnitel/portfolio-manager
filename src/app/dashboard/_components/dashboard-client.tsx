    ...
    <Card className="order-1 min-w-0 md:order-1">
      ...
      {pnl !== null ? (
        <div className="mt-5 border-t border-border pt-4">
          ...
          <p className={cn("mt-1 font-semibold", Number(pnl) >= 0 ? "text-success" : "text-destructive")}>{formatSignedCurrency(pnl, dashboard.valuation.currency)}</p>
        </div>
...
function AllocationCard({ dashboard }: { dashboard: DashboardReadModel }) {
  const hasValue = dashboard.allocation.some((item) => Number(item.value) > 0);
  return (
    <Card className="h-full min-w-0">
...
function ContributionCard({ amount, projection, error, isPending, onAmount, onReasons }: { amount: string; projection: ContributionProjection | null; error: string; isPending: boolean; onAmount: (value: string) => void; onReasons: () => void }) {
  ...
  return (
    <Card className="h-full min-w-0">
      ...
      <div className="mt-4 flex flex-col gap-2"><Link href={href} className="inline-flex h-10 items-center justify-center rounded-lg bg-primary px-4 text-sm font-medium text-white hover:bg-primary/90">Plan contribution <ArrowRight className="ml-2 h-4 w-4" /></Link><Button type="button" variant="ghost" onClick={onReasons} disabled={!projection}>Why this recommendation?</Button></div>
    </Card>
  );
}

function RecentActivity({ dashboard }: { dashboard: DashboardReadModel }) {
  return <Card><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">Recent activity</h2><p className="mt-1 text-sm text-muted">Latest five portfolio transactions.</p></div><Link href="/portfolio" className="text-sm text-primary hover:underline">View all</Link></div>{dashboard.recentActivity.length ? <div className="mt-5 divide-y divide-border">{dashboard.recentActivity.map((item) => <div key={item.id} className="flex flex-col gap-2 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex items-center gap-2"><Badge tone={item.type === "SELL" ? "destructive" : "neutral"}>{formatType(item.type)}</Badge><p className="truncate font-medium">{item.assetName} · {item.symbol}</p></div><p className="mt-1 text-xs text-muted">{item.accountName} · {new Date(item.executedAt).toLocaleDateString()}</p></div><div className="text-left sm:text-right"><p className="font-medium">{formatQuantity(item.quantity)} {item.symbol}</p><p className="mt-1 text-xs text-muted">{item.pricePerUnit ? `${formatCurrency(item.pricePerUnit, item.currency)} / unit` : "No acquisition price"}</p></div></div>)}</div> : <InlineEmpty title="No activity yet" description="Transactions will appear here after you add an initial balance or trade." />}</Card>;
}
...
function StrategyStatus({ dashboard }: { dashboard: DashboardReadModel }) {
  ...
  return <Card className={cn("border-primary/25 bg-gradient-to-br from-card to-primary/5", isAttention && "border-warning/30 from-card to-warning/5")}><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3">{status.state === "EMPTY" ? <Sparkles className="mt-1 h-5 w-5 shrink-0 text-primary" /> : isAttention ? <AlertCircle className="mt-1 h-5 w-5 shrink-0 text-warning" /> : <CheckCircle2 className="mt-1 h-5 w-5 shrink-0 text-success" />}<div><p className="text-xs uppercase tracking-wide text-muted">Strategy status{status.strategyName ? ` · ${status.strategyName}` : ""}</p><h2 className="mt-2 text-xl font-semibold">{title}</h2><p className="mt-2 max-w-3xl text-sm text-muted">{description}</p>{dashboard.valuation.isPartial ? <p className="mt-2 text-sm text-warning">Status is based on priced holdings only.</p> : null}</div></div><Link href="/assistant" className="inline-flex h-10 shrink-0 items-center justify-center rounded-lg border border-border bg-surface-strong px-4 text-sm font-medium hover:border-primary/50">Ask Assistant</Link></div></Card>;
}
...
function Modal({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) { return <div className="fixed inset-0 z-50 flex items-end bg-background/80 p-4 backdrop-blur-sm md:items-center md:justify-center" role="dialog" aria-modal="true" aria-label={title}><Card className="max-h-[90vh] w-full overflow-y-auto md:max-w-lg"><div className="flex items-start justify-between gap-4"><h2 className="text-lg font-semibold">{title}</h2><button type="button" onClick={onClose} className="rounded-lg border border-border p-2 text-muted hover:text-foreground" aria-label="Close"><X className="h-4 w-4" /></button></div><div className="mt-5">{children}</div></Card></div>; }
...
function formatQuantity(value: string) { const amount = Number(value); return Number.isFinite(amount) ? new Intl.NumberFormat("en-IE", { maximumFractionDigits: 8 }).format(amount) : value; }
